...
#####
-  Each **pod** is associated with a *single* **service account**    
...
`Api Server` `service` internally [accessible at](../../../../network/urls.md):
...
- `k exec -it my-pod -- ls` **`/var/run/secrets/kubernetes.io/serviceaccount`**
    ...
    - **`token`**: The service account **(jwt) token** 
        - `ca.crt`: The certificate of the CA that signed the API server's certificate, allowing pod to verify the API server's certificate when making HTTPS calls.
